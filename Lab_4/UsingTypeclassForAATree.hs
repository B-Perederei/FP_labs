...
-- Implementation of AATree based on abstract tree type class
instance Tree AATree where
...
-- Functions for balancing AAtree out of scope of type class
-- There are here because they are not suitable for other trees except AATree
getLvl :: AATree a -> Int
getLvl Empty = 0
...
-- Functions for balancing AATree
...
-- Functioncs for balancing AATree after deleting element
...
decreaseLvl (Node a level left (Node right levelR leftR rightR))
        ...
        where expected = minimum (getLvl left : [levelR]) + 1
decreaseLvl x = x
...
balanceDel x
        ...
        | otherwise            = let Node a level left right             = sd
...
-- MAIN
main = do
    putStrLn "Initial AA-Tree (Second number in Node is level)"
    let aatree = foldr insert Empty (reverse [6..20])
    let nodeToDelete = 8
    let nodeToInsert = 4
    print aatree
    putStrLn $ "\nExample of deleting node " ++ show nodeToDelete
    print $ task nodeToDelete aatree
    putStrLn $ "\nExample of inserting node " ++ show nodeToInsert
